...
import { X, Home, Package, ShoppingBag, Star, Store, Globe, HelpCircle, LogOut } from "lucide-react";
...
const vendorLinks = [
  ...
  { href: "/vendor/shop-settings", label: "Shop", icon: Store },
  { href: "/products", label: "Marketplace", icon: Globe },
  { href: "/account/support", label: "Help", icon: HelpCircle },
];

export function VendorMobileMenu() {
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed right-4 top-20 w-80 max-w-[calc(100vw-2rem)] bg-white border border-[var(--color-border)] rounded-xl shadow-2xl z-10 flex flex-col max-h-[calc(100vh-6rem)] overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-[var(--color-border)] bg-[var(--color-bg)]">
              <h2 className="text-base font-bold text-[var(--color-text-primary)]">Vendor Menu</h2>
              <button
                onClick={() => setIsOpen(false)}
            ...
            {/* Vertical Vendor Links List (3-4 options visible, overflow scrollable) */}
